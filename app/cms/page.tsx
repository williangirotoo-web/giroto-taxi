...
export default async function CMSPage() {
  ...
  return (
    <div className="bg-[#080808] text-white min-h-screen font-sans antialiased">
      ...
      {/* HEADER */}
      <div className="pt-28 md:pt-24 pb-12 px-4 border-b border-white/5">
        <div className="max-w-6xl mx-auto text-center">
          <p className="text-yellow-400 font-semibold uppercase tracking-wider text-sm mb-3">Serviços</p>
          <h1 className="text-4xl md:text-6xl font-extrabold mb-4">
            Táxi em <span className="text-yellow-400">Umuarama</span>
          </h1>
          <p className="text-gray-400 text-lg max-w-2xl mx-auto mb-8">
            Conheça todos os serviços da Giroto Táxi em Umuarama. Atendimento 24 horas, motoristas profissionais e conforto garantido.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href={WA_LINK} target="_blank" rel="noreferrer"
              className="flex items-center justify-center gap-2 bg-green-500 hover:bg-green-400 text-white font-bold px-8 py-4 rounded-full text-base transition-all">
              Chamar no WhatsApp
            </a>
            <a href={PHONE}
              className="flex items-center justify-center gap-2 border border-white/20 hover:border-yellow-400 hover:text-yellow-400 text-white font-semibold px-8 py-4 rounded-full text-base transition-all">
              (44) 99891-3040
            </a>
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-12 space-y-24">
        {paginas.map((pagina: any) => (
          <div key={pagina._id} className="border-b border-white/5 pb-24">
            <div className="mb-10">
              {pagina.palavraChave && (
              ...
              <h2 className="text-3xl md:text-5xl font-extrabold mb-4">{pagina.titulo}</h2>
              {pagina.metaDescricao && (
                <p className="text-gray-400 text-lg leading-relaxed">{pagina.metaDescricao}</p>
              )}
            </div>
            ...
            <div className="space-y-6">
              {pagina.titulos?.map((titulo: string, i: number) => (
                <div key={i} className="bg-white/5 border border-white/10 hover:border-yellow-400/40 rounded-2xl p-8 transition-all group">
                  <h3 className="text-xl md:text-2xl font-bold text-yellow-400 mb-4 group-hover:text-yellow-300 transition-colors">
                    {titulo}
                  </h3>
                  {pagina.descricoes?.[i] && (
